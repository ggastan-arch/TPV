"""Migracion 0008: `etiqueta_aparcada` en `venta`, columna NATIVA (`op.add_column`).

Guarda de regresion (design.md, decision "Migracion con op.add_column NATIVO"):
en SQLite, `batch_alter_table` RECREA la tabla entera y PERDERIA los triggers de
inmutabilidad (`trg_venta_no_update/no_delete`, ...). `ADD COLUMN` nativo nunca
recrea la tabla, asi que triggers e invariantes quedan intactos, y la huella de
una venta YA EMITIDA bajo el esquema anterior (revision 0007) no cambia (la
columna nueva es ajena a la huella).

Sin ORM: la fila "ya emitida" se inserta con SQL crudo (igual que
test_articulos.py hace para su propia migracion 0007) para no depender de que
los modelos Python (que YA declaran `etiqueta_aparcada` una vez implementada la
Fase 1) coincidan con el esquema fisico de la revision 0007, que aun no tiene
esa columna."""
...
def _emitir_venta_pre_0008(conn) -> dict:
    """Inserta (SQL crudo) una venta ya "emitida" bajo el esquema de la revision
    0007 -- sin `etiqueta_aparcada`, que todavia no existe -- replicando lo que
    produce `NullEngine.emit` (numeracion + registro encadenado + huella real,
    calculada con la funcion de dominio `huella_alta`)."""
...
def test_migracion_0008_no_rompe_triggers_ni_huella(tmp_path, aplicar_migraciones):
    db = tmp_path / "migracion_aparcar.db"
    url = f"sqlite:///{db}"
    ...
    with engine.begin() as conn:
        campos = _emitir_venta_pre_0008(conn)
    engine.dispose()

    aplicar_migraciones(url, "head")  # 0008_venta_etiqueta_aparcada

    engine = create_engine(url)

    # 1) columna nueva: existe, NULL-able, y NULL en la fila ya emitida.
    columnas = inspect(engine).get_columns("venta")
    columna = next((c for c in columnas if c["name"] == "etiqueta_aparcada"), None)
    assert columna is not None, "falta la columna etiqueta_aparcada en venta"
    assert columna["nullable"] is True
    with engine.connect() as conn:
        valor = conn.execute(
            text("SELECT etiqueta_aparcada FROM venta WHERE id = 1")
        ).scalar_one()
    assert valor is None

    # 2) los triggers de inmutabilidad SIGUEN presentes: `add_column` nativo no
    #    recrea la tabla (a diferencia de `batch_alter_table`).
    with engine.connect() as conn:
        nombres = {
    ...
    # 3) la huella recomputada con los MISMOS campos es identica: la columna
