...
- [x] 1.1 RED `tests/test_cierre_z_f3_sustitucion.py` (nuevo; fixtures
...
- [x] 1.2 RED (mismo fichero) `test_num_tickets_no_incluye_f3_en_conversion`:
  mismo setup; HOY `num_tickets` da 1 (solo la F3) en vez de 2 (orígenes reales)
- [x] 1.3 RED (mismo fichero) `test_cross_period_no_duplica_efectivo`: emite T,
...
- [x] 2.1 GREEN `app/infraestructura/persistencia/repositorios.py`
...
- [x] 2.2 Confirmar 1.1, 1.2 y 1.3 en GREEN

## Fase 3: Regresión (dep. Fase 2)

- [x] 3.1 `tests/test_cierre_z_generar.py` completo verde, en particular
  `test_cuadre_de_totales_y_desgloses` (Z sin ninguna F3 queda inalterado)
- [x] 3.2 `.venv/Scripts/python -m pytest` — suite completa verde, sin
...
- [x] 4.1 Confirmar que `openspec/changes/convertir-en-factura-f3/design.md` y
